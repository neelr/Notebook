...
import { slugify } from "@components/semantics";
import { local } from "../pages/api/get";

async function handleCacheAndRSS(results, options = { writeFiles: true }) {
  // Create id cache mapping
  const slugs = {};
  results.forEach((story) => {
    const slug = slugify(story.title.toLowerCase());
    slugs[slug] = story.id;
  });
...
export default handleCacheAndRSS;
