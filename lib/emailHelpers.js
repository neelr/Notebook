...
<body>
  ...
  Thanks for subscribing to my newsletter/blogposts/other! It means a lot, and I would love to hear any thoughts (just reply to this email). Also if you want to <a clicktracking=off href="https://neelr.dev">unsubscribe</a>, just reply to this email and I'll remove you from the list.
</body>
</html>
  `.trim();
}
...
async function sendEmailToList({ listId, subject, emailHtml, includeSuppression = false }) {
  ...
  // Fetch all contacts from the list. SendGrid's search endpoint caps results at 50
  // per call and provides no `next` cursor, so we paginate using `updated_at` as the
  // cursor (results come back sorted by updated_at DESC). Use `<=` + dedupe-by-id to
  // safely handle ties at page boundaries.
  const contactsById = new Map();
  let cursor = null;
  let totalContacts = null;
  while (true) {
    const baseQuery = `CONTAINS(list_ids, '${listId}')`;
    const query = cursor
      ? `${baseQuery} AND updated_at <= TIMESTAMP "${cursor}"`
      : baseQuery;

    const [pageResponse] = await sgClient.request({
      method: 'POST',
      url: '/v3/marketing/contacts/search',
      body: { query }
    });

    // Only the first (unfiltered) response's contact_count reflects the true list
    // size — subsequent responses report the count for the cursor-narrowed query.
    if (totalContacts === null) {
      totalContacts = pageResponse.body.contact_count;
    }

    const page = pageResponse.body.result || [];
    ...
    // If the page was entirely duplicates (can happen at the very end with ties).
    if (!addedNew) break;

    // Reached the total — stop early.
    if (typeof totalContacts === 'number' && contactsById.size >= totalContacts) break;

    // Advance cursor to the oldest updated_at on this page. Convert SendGrid's
    // "YYYY-MM-DD HH:MM:SS +0000 UTC" format to the ISO form the query parser expects.
    const lastUpdated = page[page.length - 1].updated_at;
    const match = lastUpdated.match(/^(\d{4}-\d{2}-\d{2}) (\d{2}:\d{2}:\d{2})/);
    if (!match) break;
    cursor = `${match[1]}T${match[2]}.000+00:00`;
  }
