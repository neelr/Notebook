...
@media (prefers-color-scheme: dark) {
  .sidenote-ref { color: #4da6ff; }
  .sidenote-ref.sidenote-highlight { background-color: rgba(77, 166, 255, 0.15); }
  .sidenote { color: #aaa; }
  .sidenote.sidenote-highlight { background-color: rgba(255, 255, 255, 0.05); }
}
@media (max-width: 1199px) and (prefers-color-scheme: dark) {
  .sidenote { border-left-color: #444; background: #2a2a2a; }
}
</style>
  `;
      ...
      let result = richText
        ...
        >
          ...
          if (
            ...
            content += `<sup class="sidenote-ref" data-sidenote="${n}" id="sidenote-ref-${n}">[${n}]</sup><span class="sidenote" data-sidenote="${n}" id="sidenote-${n}">${n}. ${noteHtml}</span>`;
          }
